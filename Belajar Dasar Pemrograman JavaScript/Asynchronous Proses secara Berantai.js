...
async function getTollAccess() {
  try {
    const beli_kartu_tol = await buyTollRoadCard(25);
    const isi_saldo_kartu_tol = await topUpBalance(beli_kartu_tol, 10);
    const gunakan_akses = await useTollRoad(isi_saldo_kartu_tol);

    return gunakan_akses;
  } catch (error) {
